...
const NoteCard = ({ note, onDelete }: Props) => {
  ...
  let updateDate;
  if (note.updatedAt) {
    updateDate = formatDate(note.updatedAt);
  }
  ...
  return (
    <div className="p-8 overflow-hidden bg-blue-900 rounded-lg relative">
      ...
      <h1 className="text-3xl text-white font-bold mb-4">{note.title}</h1>
      ...
      <Link href={`/update-note/${note._id}`}>
        <div className="w-8 h-8 bg-orange-700 hover:bg-orange-900 transition-all duration-150 rounded-full absolute top-4 cursor-pointer ring-4 flex items-center justify-center flex-col">
      ...
      <div
        ...
        className="w-8 h-8 bg-rose-700 hover:bg-rose-900 transition-all duration-150 rounded-full absolute bottom-4 cursor-pointer ring-4 flex items-center justify-center flex-col"
      >
